...
void initFonts(int cWidth, int cHeight)
{
    ...
    glm::mat4 projection = glm::ortho(0.0f, static_cast<GLfloat>(cWidth), 0.0f, static_cast<GLfloat>(cHeight));
    glUseProgram(textProgram);
...
void displayTexts() {
    ...
    std::string str = "AttractorCount = " + std::to_string(currentNumOfAttractors);
    renderText(str, leftBottom.x + charWidth, leftBottom.y + charHeight * 3, scale, yellow);

    str = "Mass = " + std::to_string(static_cast<int>(attractorMass));
    renderText(str, leftBottom.x + charWidth, leftBottom.y + charHeight * 2, scale, yellow);

    str = currentClickMode == ATTRACTOR ? "Attractor" : "Origin";
    renderText(str, leftBottom.x + charWidth, leftBottom.y + charHeight, scale, red);
}